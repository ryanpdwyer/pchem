...
def run():
    ...
    system = st.text_area("System prompt", st.session_state.system)
    ...
    if submit and picture is not None and (system != st.session_state.system or model != st.session_state.model):
        response = get_response(system, img, model)
