"""Mystery Gas B - Thermodynamics Exploration (26FA A4)"""
import copy
...
def draw(current, container):
    ...
    water_color = "#d7f1fa"
    air = patches.Rectangle((0.1, 0.1), 0.8, 0.8, linewidth=1, ec="0", fc='1')

    waterCu = patches.Rectangle((0.1, 0.1), 0.8, 0.8, linewidth=3, ec="#b87333", fc='1')

    container_dict = {
        "Dewar": [patches.Rectangle((0.05, 0.05), 0.9, 0.9, linewidth=1, ec="0", fc='1'), air],
        "Styrofoam": [patches.Rectangle((0.05, 0.05), 0.9, 0.9, linewidth=1, ec="0", fc='0.9'), air],
...
def Pressure_bar(m_gas, T_Celsius):
    return CP.PropsSI('P', 'D', m_gas/Volume, 'T', T_Celsius+273.15, FLUID)/1e5


def simulate(Tsys, Tsurr, current, work, container, m_gas):
    mMass = CP.PropsSI('M', FLUID)*1000
    n_gas = m_gas / mMass
    c = container
    work += current**2 * dt
    density = m_gas/Volume
    cp = m_gas/1000 * CP.PropsSI('Cvmass', 'D', density, 'T', Tsys+273.15, FLUID)
...
data_default = dict(t=[0], Tsys=[20.0], work=[0], m=[1.0])

st.markdown("""# Thermodynamics of Mystery Gas B
...
if 'runningB' not in st.session_state:
    st.session_state.runningB = False
...
if 'containerB' not in st.session_state:
    st.session_state.containerB = "Dewar"

if 'thermGasBData' not in st.session_state:
...
st.session_state.containerB = containers[container]

button_text = "Pause" if st.session_state.runningB else "Run"
...
if reset_simulation:
    st.session_state.runningB = False
    st.session_state.thermGasBData = copy.copy(data_default)
    st.session_state.containerB = "Dewar"
...
work = st.session_state.thermGasBData["work"][-1]
Tsys = st.session_state.thermGasBData["Tsys"][-1]
...
if show_data:
    df = pd.DataFrame(st.session_state.thermGasBData)
    ...
    filename = st.text_input("Filename:", value="CHE341-gas-B-data")
...
if st.session_state.runningB:
    work, Tsys = simulate(Tsys, Tsurr, current, work, st.session_state.containerB, m_gas)
    st.session_state.thermGasBData["work"].append(work)
    st.session_state.thermGasBData["Tsys"].append(Tsys)
    st.session_state.thermGasBData['m'].append(m_gas)
    st.session_state.thermGasBData['t'].append(st.session_state.thermGasBData['t'][-1]+dt)
