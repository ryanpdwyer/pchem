"""Electron Visualization - Radial Probability Distributions"""
import pandas as pd
import numpy as np
import json
...
from pchemapps.base import elements_df, get_e_config, n_in_shell, Zeff, write_e_config, write_ion
...
for shell, e_in_shell in shells.items():
    ...
    for subshell, e_in_subshell in subshells.items():
        ...
        df = pd.concat([df, pd.DataFrame(dict(r=r[mask], p=p_rad[mask]/dr, n=shell, l=l_subshell, subshell=f"{shell}{subshell}"))])
