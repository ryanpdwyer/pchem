...
def to_gaussian_input(
    mol: Chem.Mol,
    route: str = "# PM6 SP",
    title: str = "Oligomer generated by pchem",
    charge: int = 0,
    multiplicity: int = 1,
    conf_id: int = 0,
) -> str:
    """Return a Gaussian input file for an existing 3D conformer."""
    if mol.GetNumConformers() == 0:
        raise ValueError("A 3D conformer is required to create Gaussian input.")
    route = route.strip()
    if not route:
        raise ValueError("Gaussian route section is empty.")
    ...
    for atom in mol.GetAtoms():
        ...
        coordinates.append(
            f"{atom.GetSymbol():<2} {pos.x: .10f} {pos.y: .10f} {pos.z: .10f}"
        )
    clean_title = " ".join((title or "Oligomer generated by pchem").splitlines())
    return (
        f"{route}\n\n{clean_title}\n\n{charge} {multiplicity}\n"
        + "\n".join(coordinates)
        + "\n\n"
